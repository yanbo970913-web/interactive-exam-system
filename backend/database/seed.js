...
function runSeed() {
  ...
  // ── 2. 建立管理者帳號（密碼從環境變數讀取，絕不硬編碼）
  const adminUsername = process.env.ADMIN_USERNAME;
  const adminPassword = process.env.ADMIN_PASSWORD;
  if (!adminUsername || !adminPassword) {
    console.error('❌ 請在 .env 檔案中設定 ADMIN_USERNAME 與 ADMIN_PASSWORD 後再執行 seed');
    console.error('   範例：ADMIN_USERNAME=admin  ADMIN_PASSWORD=your_secure_password');
    process.exit(1);
  }
  ...
  const insertUser = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `);
  const adminResult = insertUser.run(adminUsername, adminPasswordHash, process.env.ADMIN_DISPLAY_NAME || '系統管理員', 'admin', '#7C3AED');
  ...
  for (const vocab of vocabulary) {
    ...
    // 選擇題：看英文選中文
    const sameLevelWords = vocabulary.filter(v => v.level === vocab.level && v.word !== vocab.word);
    const distractors = shuffleArray(sameLevelWords).slice(0, 3).map(v => v.translation);
    const allOptions = shuffleArray([vocab.translation, ...distractors]);
    insertQuestion.run(
      engSubjectId, vocab.level, 'choice',
  ...
  for (const q of pythonQuestions) {
    insertQuestion.run(pySubjectId, q.level, q.type, q.question_text, q.options || null, q.correct_answer, q.explanation, adminId);
  }
  console.log(`  ✅ Python 程式設計題目建立完成：${pythonQuestions.length} 題`);

  // ── 6. 數位邏輯題目
  for (const q of digitalLogicQuestions) {
    insertQuestion.run(dlSubjectId, q.level, q.type, q.question_text, q.options || null, q.correct_answer, q.explanation, adminId);
  }
  ...
  const insertExam = db.prepare(`
    INSERT INTO exams (title, description, subject_id, level_filter, question_count, start_time, end_time, duration_minutes, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  insertExam.run('Level 1-2 英文單字小測驗', '測試 Level 1 與 Level 2 基礎英文單字。', engSubjectId, '1,2', 10, now.toISOString(), nextWeek.toISOString(), 15, adminId);
  insertExam.run('Python 基礎概念測驗', '測試 Python 基礎語法、資料型別與基本操作。', pySubjectId, 'all', 10, now.toISOString(), nextWeek.toISOString(), 20, adminId);
  insertExam.run('數位邏輯基礎測驗', '測試布林代數、邏輯閘真值表與進制轉換。', dlSubjectId, 'all', 10, now.toISOString(), nextWeek.toISOString(), 20, adminId);
  console.log('  ✅ 示範考試建立完成：3 場考試');
}
...
try {
  ...
  console.log(`  管理者帳號: ${process.env.ADMIN_USERNAME}`);
  console.log('─'.repeat(50));
  console.log('\n▶  執行 npm start 啟動伺服器');
} catch (err) {
  db.exec('ROLLBACK');
