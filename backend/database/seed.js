...
function runSeed() {
  ...
  const insertExam = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  insertExam.run('Level 1-2 英文單字小測驗', '測試 Level 1 與 Level 2 基礎英文單字。',
    engSubjectId, '1,2', 10, now.toISOString(), nextYear.toISOString(), 15, adminId);
  insertExam.run('Python 基礎概念測驗', '測試 Python 基礎語法、資料型別與基本操作。',
    pySubjectId, 'all', 10, now.toISOString(), nextYear.toISOString(), 20, adminId);
  insertExam.run('數位邏輯基礎測驗', '測試布林代數、邏輯閘真值表與進制轉換。',
    dlSubjectId, 'all', 10, now.toISOString(), nextYear.toISOString(), 20, adminId);
